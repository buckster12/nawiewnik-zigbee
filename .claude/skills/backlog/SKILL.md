---
name: backlog
description: Read, work, and maintain a Git repo's deferred-work items in backlog/, one file per item. Use when the user says "backlog", "check backlog", "what's on my backlog", "work the backlog", "address the backlog", "add to backlog", "clean up backlog", or when a review or task produced items that are real but not being fixed now. Owns the item format and the create-then-delete lifecycle.
allowed-tools: Read, Edit, Write, Bash, Grep, Glob, AskUserQuestion
---

# Backlog

`backlog/` at a repo root holds work that is real but not being done now: a defect a change did not
...
`backlog/<slug>.md`. The slug names the defect, not the file it lives in
...
`/backlog --all` walks every current item to a disposition. Check this mode before slug handling.
...
1. Glob `backlog/*.md` from the repo root and read every file in full. Keep `worth: no` items in the
...
`/backlog <slug>` names one item: `backlog/<slug>.md`, the file name without its extension. Read
...
1. Glob `backlog/*.md` from the repo root and read each file's frontmatter and H1. If the directory
...
Create `backlog/` if it does not exist — after the branch check above, never before it.
